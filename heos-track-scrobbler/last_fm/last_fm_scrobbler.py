...
class LastFmScrobbler(object):
    ...
    try:
      ...
      if last_fm_user:
        if last_fm_user.username and last_fm_user.password and not last_fm_user.sessionKey:
          print("Setting Last.fm session key for user {}".format(last_fm_user.username))
          last_fm_user = self._set_last_fm_session_key(last_fm_user)
          print("Set Last.fm session key for user {}".format(last_fm_user.username))
        elif not last_fm_user.sessionKey and (not last_fm_user.username or not last_fm_user.password):
          print("Last.fm user missing username or password, set them to scrobble")
      ...
      self.creating_user = False
      self._log_error(
        "Error occured while getting user",
  ...
  def _get_last_fm_network(self, last_fm_user=None):
    try:
      ...
      self._log_error(
        "Error occured while getting Last.fm network",
        traceback.format_exc()
      )
  ...
  def _set_last_fm_session_key(self, last_fm_user):
    try:
      ...
      session_key = pylast.SessionKeyGenerator(last_fm_network).get_session_key(
        ...
        pylast.md5(last_fm_user.password)
      )
  ...
  def _scrobble_length_is_valid(self, last_fm_user, heos_played_track):
    if heos_played_track.duration:
      ...
      if listening_duration < last_fm_user.minimumScrobbleLength * heos_played_track.duration:
        return False
      if listening_duration > last_fm_user.maximumScrobbleLength * heos_played_track.duration:
        return False
    return True
    ...
    try:
      ...
      for heos_played_track in heos_played_tracks:
        try:
          if (last_fm_user and
              last_fm_network and
              self._required_scrobble_data_exists(heos_played_track, is_scrobble=True) is True and
              self._scrobble_length_is_valid(last_fm_user, heos_played_track) is True):
